...
resolver.resolveInstanceField = function(receiverNode, fieldName, conditionState) {
  let classNode
  ...
  if (classNode.superClass) {
    ...
    const instanceField = this.resolveInstanceField(classNode.superClass, fieldName, conditionState);
    if (instanceField) return instanceField
  }
}
...
resolver.reduce = function(init, fieldNames, conditionState) {
  ...
  for (let i = 0; i < fieldNames.length - 1; i++) {
    ...
    const instanceField = this.resolveInstanceField(receiverNode, fieldName, conditionState)
    if (!instanceField) return null;
    receiverNode = instanceField.type;
  }

  let key = fieldNames[fieldNames.length - 1]
  const instanceField = this.resolveInstanceField(receiverNode, key, conditionState)
