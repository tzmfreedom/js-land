...
const methodResolver = require('../type/method-resolver')
const variableResolver = require('../type/variable-resolver')
...
class SoqlNode {
  ...
  toString(visitor) {
    ...
    const where = builder.visit(this.where)
    builder.select(this)
    console.log(builder.joins)
    let whereFields = ""
    if (where.length > 0) whereFields = `WHERE ${where}`
    const soql = `SELECT ${selectFields} FROM ${this.fromObject.toUpperCase()} ${whereFields}`
    console.log(soql)
    return soql
  }
}
