...
class SoqlNode {
  ...
  type() {
    const objectType = new TypeNode([this.fromObject], [])
    objectType.classNode = NameSpaceStore.get('System', this.fromObject)

    const type = new TypeNode(
      ['List'],
      [objectType]
    )
    type.classNode = NameSpaceStore.get('System', 'List')
    return type
  }
