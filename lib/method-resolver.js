...
class MethodResolver {
  _resolve(node) {
    ...
    if (EnvManager.localIncludes(name)) {
      ...
      let receiverNode = this.reduce(variable, names.slice(1, names.length-1), conditionState, node);
      if (receiverNode) {
        ...
        let methodNode = this.resolveInstanceMethod(receiverNode, methodName, node.parameters, conditionState);
    ...
    if (EnvManager.localIncludes('this')) {
      ...
      let receiverNode = this.reduce(variable, names.slice(0, names.length-1), modifier.PRIVATE, node);
      if (receiverNode) {
        const conditionState = (() => {
          if (names.length === 1) {
            return modifier.PRIVATE
    ...
    if (names.length == 1) {
      if (EnvManager.localIncludes('_class')) {
        ...
        if (classNode) {
          let methodNode = this.resolveStaticMethod(classNode, name, node.parameters, modifier.PRIVATE);
          if (methodNode) {
    ...
    if (names.length == 2) {
      ...
      if (classInfo) {
        let methodNode = this.resolveStaticMethod(classInfo, names[1], node.parameters, modifier.PUBLIC);
    ...
    if (names.length >= 3) {
      ...
      if (classNode) {
        ...
        if (classNode.staticFields.includes(staticFieldName)) {
          ...
          let receiverNode = this.reduce(field, names.slice(2, names.length-1), modifier.PUBLIC, node);
          if (receiverNode) {
            let methodNode = this.resolveInstanceMethod(receiverNode, methodName, node.parameters, modifier.PUBLIC);
            if (receiverNode && methodNode) {
              if (!methodNode.isPublic()) {
                throw `Method is not visible: ${methodNode.name} at line ${node.lineno}`;
              }
    ...
    if (names.length >= 3) {
      ...
      if (classNode) {
        ...
        if (classNode.staticFields.includes(staticFieldName)) {
          const field = ClassStaticFields.get(classNode.name, staticFieldName);
          let receiverNode = this.reduce(field, names.slice(3, names.length-1), modifier.PUBLIC, node);
        ...
        if (names.length == 3) {
          let methodNode = this.resolveStaticMethod(classNode, names[2], node.parameters, modifier.PUBLIC);
          if (methodNode) {
            return new MethodResolveResult(classNode, methodNode);
          }
        }
    ...
    throw `Method is not exist ${names.join('.')} at line ${node.lineno}`
  }

  resolveInstanceMethod(receiverNode, methodName, parameters, conditionState) {
    let classNode
    ...
    if (classNode.superClass) {
      ...
      const methodNode = this.resolveInstanceMethod(classNode.superClass, methodName, parameters, conditionState);
      if (methodNode) {
        this.checkMethodModifier(receiverNode, methodNode, conditionState)
        return methodNode
      }
