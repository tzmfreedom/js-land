...
class MethodResolver {
  searchMethod(node, reduce) {
    ...
    if (names.length >= 3) {
      ...
      if (classNode) {
        ...
        if (names.length == 3) {
          let methodNode = this.searchStaticMethod(classNode, names[2], node.parameters);
          if (methodNode) {
            return new MethodSearchResult(classNode, methodNode);
          }
        }
  ...
  searchInstanceMethod(receiverNode, methodName, parameters) {
    ...
    if (classNode.superClass) {
      const methodNode = this.searchInstanceMethod(classNode.superClass, methodName, parameters);
      if (methodNode) return methodNode
    }
  }
  ...
  searchStaticMethod(classNode, methodName, parameters) {
    const methodNodes = (() => {
      if (classNode.staticMethods.includes(methodName)) {
        return classNode.staticMethods.get(methodName);
      } else if (classNode.superClass) {
        return this.searchStaticMethod(classNode.superClass, methodName, parameters);
      } else {
        return null;
      }
    })();
