...
function reduceTypeByInstanceField(init, fieldNames, conditionState) {
  ...
  for (let i = 0; i < fieldNames.length; i++) {
    ...
    conditionState = condition.PUBLIC
...
class MethodResolver {
  searchMethod(node, reduce) {
    ...
    if (EnvManager.localIncludes(name)) {
      ...
      const conditionState = name === 'this' ? condition.PRIVATE : condition.PUBLIC
      let receiverNode = reduce(variable, names.slice(1, names.length-1), conditionState);
      if (receiverNode) {
    ...
    if (EnvManager.localIncludes('this')) {
      ...
      let receiverNode = reduce(variable, names.slice(0, names.length-1), condition.PRIVATE);
      if (receiverNode) {
        const conditionState = (() => {
          if (names.length === 1) {
            return condition.PRIVATE
    ...
    if (names.length >= 3) {
      ...
      if (classNode) {
        ...
        if (classNode.staticFields.includes(staticFieldName)) {
          ...
          let receiverNode = reduce(field, names.slice(2, names.length-1), condition.PUBLIC);
          if (receiverNode) {
    ...
    if (names.length >= 3) {
      ...
      if (classNode) {
        ...
        if (classNode.staticFields.includes(staticFieldName)) {
          const field = ClassStaticFields.get(classNode.name, staticFieldName);
          let receiverNode = reduce(field, names.slice(3, names.length-1), condition.PUBLIC);
  ...
  checkMethodModifier(receiverNode, methodNode, conditionState) {
    if (conditionState === condition.PUBLIC && !methodNode.isPublic()) {
      throw `Method is not visible: ${methodNode.name} at line ${receiverNode.lineno}`;
    }
    if (conditionState === condition.PROTECTED && methodNode.isPrivate()) {
      throw `Method is not visible: ${methodNode.name} at line ${receiverNode.lineno}`;
    }
  }
}
