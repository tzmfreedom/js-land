...
class MethodResolver {
  _resolve(node) {
    ...
    if (EnvManager.localIncludes(name)) {
      ...
      let receiverNode = this.reduce(variable, names.slice(1, names.length-1), conditionState);
      if (receiverNode) {
    ...
    if (EnvManager.localIncludes('this')) {
      ...
      let receiverNode = this.reduce(variable, names.slice(0, names.length-1), modifier.PRIVATE);
      if (receiverNode) {
        const conditionState = (() => {
          if (names.length === 1) {
            return modifier.PRIVATE
    ...
    // Class.static_method()
    if (names.length == 2) {
      console.log(ApexClassStore.all())
    ...
    if (names.length >= 3) {
      ...
      if (classNode) {
        ...
        if (classNode.staticFields.includes(staticFieldName)) {
          ...
          let receiverNode = this.reduce(field, names.slice(2, names.length-1), modifier.PUBLIC);
          if (receiverNode) {
    ...
    if (names.length >= 3) {
      ...
      if (classNode) {
        ...
        if (classNode.staticFields.includes(staticFieldName)) {
          const field = ClassStaticFields.get(classNode.name, staticFieldName);
          let receiverNode = this.reduce(field, names.slice(3, names.length-1), modifier.PUBLIC);
