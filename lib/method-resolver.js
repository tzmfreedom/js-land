...
class MethodResolver {
  _resolve(node) {
    ...
    if (EnvManager.localIncludes(name)) {
      ...
      if (receiverNode) {
        ...
        let methodNode = this.resolveInstanceMethod(receiverNode, methodName, node.parameters, conditionState, node)
    ...
    if (names.length >= 3) {
      ...
      if (classNode) {
        ...
        if (classNode.staticFields.includes(staticFieldName)) {
          ...
          if (receiverNode) {
            let methodNode = this.resolveInstanceMethod(receiverNode, methodName, node.parameters, modifier.PUBLIC, node)
            if (receiverNode && methodNode) {
              if (!methodNode.isPublic()) {
                throw `Method is not visible: ${methodNode.name} at line ${node.lineno}`;
              }
  ...
  resolveInstanceMethod(receiverNode, methodName, parameters, conditionState, node) {
    if (receiverNode instanceof Ast.NullNode) {
      throw `Null pointer exception : ${methodName}() at line ${node.lineno}`
    }
    let classNode
