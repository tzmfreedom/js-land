...
class DataManager {
  ...
  load(soql) {
    ...
    if (this.records.includes(soql.fromObject)) {
      records = this.records.get(soql.fromObject)
    } else {
      this.records.set(soql.fromObject, this.loadFunction.call(this, soql.fromObject))
      records = this.records.get(soql.fromObject)
    }
    listObject._records = records.filter((record) => {
      return soql.where.every((where) => {
        const fieldPath = where.field.path.join('.')
        return record.instanceFields[fieldPath].value == soql.where[fieldPath]
      })
    })
