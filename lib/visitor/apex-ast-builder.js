...
ApexAstBuilder.prototype.visitTypeDeclaration = function(ctx) {
  ...
  if (ctx.classDeclaration()) {
    const classDeclaration = ctx.classDeclaration().accept(this);
    classDeclaration.modifiers = modifiers
    return classDeclaration;
  } else if (ctx.enumDeclaration()) {
    const enumDeclaration = ctx.enumDeclaration().accept(this);
    enumDeclaration.modifiers = modifiers;
    return enumDeclaration;
  } else if (ctx.interfaceDeclaration()) {
    const interfaceDeclaration = ctx.interfaceDeclaration().accept(this);
    interfaceDeclaration.modifiers = modifiers;
...
ApexAstBuilder.prototype.visitClassDeclaration = function(ctx) {
  const className = ctx.apexIdentifier().getText();
  let superClass;
  if (ctx.type()) {
  ...
  const declarations = ctx.classBody().accept(this);
  const constructor = declarations.filter((declaration) => {
    return declaration instanceof Ast.ConstructorDeclarationNode;
  });
  const instanceMethods = declarations.filter((declaration) => {
    return (
  ...
  const staticMethods = declarations.filter((declaration) => {
    return (
      declaration instanceof Ast.MethodDeclarationNode
  ...
  const instanceFields = declarations.filter((declaration) => {
    return (
  ...
  const staticFields = declarations.filter((declaration) => {
    return (
      declaration instanceof Ast.FieldDeclarationNode
      && declaration.modifiers.some((modifier) => {
        return modifier.name == 'static';
  ...
  const instanceProperties = declarations.filter((declaration) => {
    return (
  ...
  const staticProperties = declarations.filter((declaration) => {
    return (
      declaration instanceof Ast.PropertyDeclarationNode
      && declaration.modifiers.some((modifier) => {
        return modifier.name == 'static';
  ...
  const innerClasses = declarations.filter((declaration) => {
...
ApexAstBuilder.prototype.visitInterfaceDeclaration = function(ctx) {
  const identifier = ctx.apexIdentifier().accept(this)
  const body = ctx.interfaceBody().accept(this)
  const instanceMethods = body.filter((declaration) => {
    return (
      declaration instanceof Ast.MethodDeclarationNode
      && declaration.modifiers.every((modifier) => {
        return modifier.name != 'static';
  ...
  const staticMethods = body.filter((declaration) => {
    return (
      declaration instanceof Ast.MethodDeclarationNode
      && declaration.modifiers.some((modifier) => {
        return modifier.name == 'static';
...
ApexAstBuilder.prototype.visitInterfaceBody = function(ctx) {
  return ctx.interfaceBodyDeclaration().map((declaration) => {
    return declaration.accept(this)
  })
};
...
ApexAstBuilder.prototype.visitClassBodyDeclaration = function(ctx) {
  const declaration = ctx.memberDeclaration();
  if (declaration) {
    const decl = declaration.accept(this);
...
ApexAstBuilder.prototype.visitMethodDeclaration = function(ctx) {
  const methodName = ctx.apexIdentifier().getText();
  const returnType = ctx.type() ? ctx.type().accept(this) : 'void';
  const parameters = ctx.formalParameters().accept(this);
  const throws = ctx.qualifiedNameList() ? ctx.qualifiedNameList().accept(this) : [];
  const statements = ctx.methodBody() ? ctx.methodBody().accept(this) : null;
...
ApexAstBuilder.prototype.visitConstructorDeclaration = function(ctx) {
  const name = ctx.apexIdentifier().accept(this);
  const formalParameters = ctx.formalParameters().accept(this);
  const throws = ctx.qualifiedNameList() ? ctx.qualifiedNameList().accept(this) : [];
  const constructorBody = ctx.constructorBody().accept(this);
...
ApexAstBuilder.prototype.visitFieldDeclaration = function(ctx) {
  ...
  const declarators = ctx.variableDeclarators().accept(this);
...
ApexAstBuilder.prototype.visitInterfaceBodyDeclaration = function(ctx) {
  const declaration = ctx.interfaceMemberDeclaration().accept(this)
  declaration.modifiers = ctx.modifier().map((modifier) => {
    return modifier.accept(this)
  })
  declaration.modifiers.push(new Ast.ModifierNode('public', ctx.start.line))
  return declaration
};
...
ApexAstBuilder.prototype.visitConstantDeclarator = function(ctx) {
  const identifier = ctx.apexIdentifier().accept(this)
  const variableInitializer = ctx.variableInitializer().accept(this)

  // TODO: const declarator
};
...
ApexAstBuilder.prototype.visitInterfaceMethodDeclaration = function(ctx) {
  const methodName = ctx.apexIdentifier().accept(this)
  const returnType = ctx.type() ? ctx.type().accept(this) : 'void'
  const parameters = ctx.formalParameters().accept(this)
  const throws = ctx.qualifiedNameList() ? ctx.qualifiedNameList().accept(this) : null

  // TODO: interfaceMethodDeclaration
  return new Ast.MethodDeclarationNode(
    methodName,
    null,
    returnType,
...
ApexAstBuilder.prototype.visitVariableDeclarator = function(ctx) {
  const declaratorId = ctx.variableDeclaratorId().getText();
  let expression;
...
ApexAstBuilder.prototype.visitClassOrInterfaceType = function(ctx) {
  if (ctx.typeIdentifier()) {
    ...
    const name = ctx.typeIdentifier().map((identifier) => {
      return identifier.getText();
    });
    return new Ast.TypeNode(name, parameters, ctx.start.line);
  } else if (ctx.SET()) {
    const parameters = ctx.typeArguments().accept(this);
...
ApexAstBuilder.prototype.visitQualifiedName = function(ctx) {
  const value = ctx.apexIdentifier().map((identifier) => {
    return identifier.getText();
  });
...
ApexAstBuilder.prototype.visitAnnotation = function(ctx) {
  const name = ctx.annotationName().accept(this);
  let parameters;
...
ApexAstBuilder.prototype.visitElementValuePair = function(ctx) {
  ...
  const expression = ctx.elementValue().accept(this);
  return {
...
ApexAstBuilder.prototype.visitStatement = function(ctx) {
  ...
  if (ctx.TRY()) {
    ...
    const catchClause = ctx.catchClause() ? ctx.catchClause().map((catchClause) => {
        return catchClause.accept(this);
    }) : null;
    const finallyBlock = ctx.finallyBlock() ? ctx.finallyBlock().accept(this) : null;
    return new Ast.TryNode(block, catchClause, finallyBlock, lineNo);
  } else if (ctx.IF()) {
    const condition = ctx.parExpression().accept(this);
    const if_statement = ctx.statement()[0].accept(this);
    const else_statement = ctx.statement().length > 1 ? ctx.statement()[1].accept(this) : null;
    ...
    const forControl = ctx.forControl().accept(this);
    const statement = ctx.statement()[0].accept(this);
    return new Ast.ForNode(forControl, statement, lineNo);
  } else if (ctx.WHILE()) {
    const doFlag = ctx.DO() != null;
    const condition = ctx.parExpression().accept(this);
    const statements = ctx.statement().map((statement) => {
      return statement.accept(this)
    });
    return new Ast.WhileNode(condition, statements, doFlag, lineNo);
  } else if (ctx.RETURN()) {
    const expression = ctx.expression() ? ctx.expression().accept(this) : new Ast.NullNode(ctx.start.line);
...
ApexAstBuilder.prototype.visitCatchClause = function(ctx) {
  ...
  const catchType = ctx.catchType().accept(this);
  const block = ctx.block().accept(this);
  const identifier = ctx.apexIdentifier().getText();
...
ApexAstBuilder.prototype.visitForControl = function(ctx) {
  if (ctx.enhancedForControl()) {
    ...
    const forInit = ctx.forInit() ? ctx.forInit().accept(this) : null;
    const expression = ctx.expression() ? ctx.expression().accept(this) : null;
    const forUpdate = ctx.forUpdate() ? ctx.forUpdate().accept(this) : null;
...
ApexAstBuilder.prototype.visitApexDbExpressionShort = function(ctx) {
  const dml = ctx.dml ? ctx.dml.text : 'upsert';
  const expression = ctx.expression().accept(this);
  const upsertKey = ctx.apexIdentifier() ? ctx.apexIdentifier().accept(this) : null
...
ApexAstBuilder.prototype.visitOpExpression = function(ctx) {
  ...
  const right = ctx.expression().length > 1 ? ctx.expression()[1].accept(this) : ctx.type().accept(this);
...
ApexAstBuilder.prototype.visitMethodInvocation = function(ctx) {
  const receiver = ctx.expression().accept(this);
  let arguments = [];
...
ApexAstBuilder.prototype.visitShiftExpression = function(ctx) {
  ...
  const op = ctx.op.map((op) => op.text ).join('')
  return new Ast.BinaryOperatorNode(op, left, right, ctx.start.line)
};
...
ApexAstBuilder.prototype.visitFieldAccess = function(ctx) {
  const expression = ctx.expression().accept(this);
  const fieldName = ctx.apexIdentifier().accept(this);
...
ApexAstBuilder.prototype.visitCreator = function(ctx) {
  if (ctx.nonWildcardTypeArguments()) {
    const createdName = ctx.createdName().accept(this);
    const classCreatorRest = ctx.classCreatorRest().accept(this);
    return new Ast.NewNode(createdName, classCreatorRest, ctx.start.line);
  } else {
    const createdName = ctx.createdName().accept(this);
    const arrayCreatorRest = ctx.arrayCreatorRest() ? ctx.arrayCreatorRest().accept(this) : null;
    const classCreatorRest = ctx.classCreatorRest() ? ctx.classCreatorRest().accept(this) : null;
    const mapCreatorRest = ctx.mapCreatorRest() ? ctx.mapCreatorRest().accept(this) : null;
    const setCreatorRest = ctx.setCreatorRest() ? ctx.setCreatorRest().accept(this) : null;
...
ApexAstBuilder.prototype.visitCreatedName = function(ctx) {
  if (ctx.apexIdentifier()) {
    const name = ctx.apexIdentifier().map((identifier) => {
      return identifier.getText();
    });
    const arguments = ctx.typeArgumentsOrDiamond().length > 0 ? ctx.typeArgumentsOrDiamond()[0].accept(this) : [];
...
ApexAstBuilder.prototype.visitQuery = function(ctx) {
  ...
  const limit        = ctx.limitClause() ? ctx.limitClause().accept(this) : null
...
ApexAstBuilder.prototype.visitWhereClause = function(ctx) {
  return ctx.whereFields().accept(this)
}

ApexAstBuilder.prototype.visitWhereFields = function(ctx) {
  if (ctx.whereField()) {
    return ctx.whereField().accept(this)
  }
  const left = ctx.whereFields()[0].accept(this)
  const logical = ctx.and_or.text
  const right = ctx.whereFields()[1].accept(this)
