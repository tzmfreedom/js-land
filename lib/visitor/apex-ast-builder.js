...
ApexAstBuilder.prototype.visitWhereClause = function(ctx) {
  const whereFields = ctx.whereField()
  const headNode = {
    type: 'whereLogical',
    left: whereFields[0].accept(this),
    logical: '',
    right: null
  }
  let currentNode = headNode
  for (let i = 1; i < whereFields.length-1; i++) {
    const whereField = whereFields[i]
    const tmpNode = {
      type: 'whereLogical',
      left: whereField.accept(this),
      logical: ctx.and_or.text,
      right: null
    }
  ...
  if (whereFields.length > 1) {
    const whereField = whereFields[whereFields.length - 1]
    currentNode.right = whereField.accept(this)
    console.log(ctx.and_or)
  }
  return headNode
}
