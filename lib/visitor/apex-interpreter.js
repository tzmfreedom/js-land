...
class ApexInterpreter {
  ...
  invokeMethodByNoParameter(methodNode, expression) {
    EnvManager.pushScope({ this: new Variable(null, expression) });
    let returnNode;
  ...
  visitNew(node) {
    ...
    let env = {
      this: new Variable(node.classType, newObject)
    };
    if (constructor) {
    ...
    switch(node.op) {
      ...
      case '^=':
        if (node.right instanceof Ast.ArrayAccessNode) {
          ...
          if (result.key) {
            result.receiverNode.instanceFields[result.key] = new Ast.IntegerNode(left.value + right.value);
          } else {
            EnvManager.setValue(result.receiverNode, new Ast.IntegerNode(left.value + right.value));
          }
        }
        return right;
    }
