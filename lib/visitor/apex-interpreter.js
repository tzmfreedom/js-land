...
class ApexInterpreter {
  ...
  visitName(node) {
    let result = variableResolver.resolve(node, this);
    if (result.key) {
  ...
  visitUnaryOperator(node) {
    let result = variableResolver.resolve(node.expression, this);
    let prev, value;
    if (result.key) {
    ...
    switch(node.op) {
      ...
      case '^=':
        if (node.left instanceof Ast.ArrayAccessNode) {
          ...
          let result = variableResolver.resolve(node.left, this);
