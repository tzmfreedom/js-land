...
class ApexInterpreter {
  ...
  visitBinaryOperator(node) {
    ...
    const klass = Ast[`${node.left.type().name.join('.')}Node`]
    switch(node.op) {
      case '+':
      ...
      case '^=':
        if (node.left instanceof Ast.ArrayAccessNode) {
          ...
          receiver._records[key.value] = right.value;
          switch(node.op) {
            case '=':
              receiver._records[key.value] = new klass(right.value);
              break
            case '+=':
          ...
          if (result.key) {
            switch(node.op) {
              case '=':
                result.receiverNode.instanceFields[result.key] = new klass(right.value);
                break
              case '+=':
                result.receiverNode.instanceFields[result.key] = new klass(left.value + right.value);
                break
              case '-=':
                result.receiverNode.instanceFields[result.key] = new klass(left.value - right.value);
                break
  ...
  visitSoql(node) {
    return dataManager.load(node.soql)
  }
