...
const methodResolver = require('../resolver/interpreter/method-resolver')
const variableResolver = require('../resolver/interpreter/variable-resolver')
const EnvManager = require('../env-manager')
const dataManager = require('../database/data-manager')
const DebuggerPublisher = require('../event/debugger-publisher')
...
class ApexInterpreter {
  ...
  visitEnhancedFor (node) {
    ...
    const hasNextInstanceMethod = expression.type().classNode.instanceMethods.get('hasNext')[0]
    const nextInstanceMethod = expression.type().classNode.instanceMethods.get('next')[0]
  ...
  visitMethodInvocation (node) {
    ...
    if (node.nameOrExpression instanceof Ast.NameNode) {
      if (node.nameOrExpression.value[0] == '_Debugger') {
        this.debug(node)
        return null
      }
    }
    const searchResult = methodResolver.resolve(node, this)
  ...
  visitName (node) {
    const result = variableResolver.resolve(node, this)
    if (result.key) {
  ...
  visitNew (node) {
    ...
    if (node.classType.classNode.superClass) {
      const instanceFields = node.classType.classNode.superClass.classNode.instanceFields
      instanceFields.keys().forEach((fieldName) => {
        newObject.instanceFields.set(fieldName, instanceFields.get(fieldName).expression.accept(this))
      })
    }
  ...
  visitUnaryOperator (node) {
    const result = variableResolver.resolve(node.expression, this)
    let prev, value
    if (result.key) {
    ...
    switch (node.op) {
      ...
      case '^=':
        if (node.left instanceof Ast.ArrayAccessNode) {
          ...
          const result = variableResolver.resolve(node.left, this)
          // FieldAccess
          if (result.key) {
            switch (node.op) {
              case '=':
  ...
  visitVariableDeclaration (node) {
    ...
    node.declarators.forEach((declarator) => {
      const value = declarator.expression.accept(this)
      const env = EnvManager.currentScope()
  ...
  debug (node) {
    ...
    const subscriberId = DebuggerPublisher.addSubscriber('line', (event) => {
      if (step > 0) step -= 1
      node.filename = 'examples/sample.cls'
      ...
      next: (args) => {
        ...
        const subscriberId = DebuggerPublisher.addSubscriber('line', (event) => {
          ...
          if (step == 0) {
            ...
            event.filename = 'examples/sample.cls'
