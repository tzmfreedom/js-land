...
class ApexBuilder {
  ...
  visitClass(node) {
    ...
    staticMethods.keys().forEach((methodName) => {
      let methods = staticMethods.get(methodName);
    ...
    node.instanceMethods.keys().forEach((methodName) => {
      let methods = instanceMethods.get(methodName);
      methods.forEach((methodNode) => {
        let env = {
          this: {
            type: newObject.type(),
    ...
    node.implementClasses.forEach((interfaceClass) => {
      ...
      methods.keys().forEach((methodName) => {
        methods.get(methodName).forEach((method) => {
          const throwError = () => {
            const typeName = interfaceClass.returnType instanceof Ast.TypeNode ? interfaceClass.returnType.name.join('.') : 'void'
            const params = method.parameters.map((parameter) => {
              return parameter.type().name.join('.')
            }).join(', ')
            throw `Class ${node.name} must implement the method: ${typeName} ${interfaceClass.name}.${methodName}(${params})`
          }
          ...
          if (!methodNode.isPublic()) {
            throw `Overriding implementations of global or public interface methods must be global or public: ${typeName} ${interfaceClass.name}.${methodName}()`
          }
        })
      })
    })
        ...
        instanceMethods.keys().forEach((methodName) => {
          instanceMethods.get(methodName).forEach((method) => {
            if (method.isAbstract()) {
              const throwError = () => {
                const typeName = method.returnType instanceof Ast.TypeNode ? method.returnType.name.join('.') : 'void'
                const params = method.parameters.map((parameter) => {
                  return parameter.type().name.join('.')
                }).join(', ')
                throw `Class ${node.name} must implement the method: ${typeName} ${method.name}.${methodName}(${params})`
              }
  ...
  createObject(classNode) {
    ...
    let instanceFields = new CaseIgnoredStore();
    EnvManager.pushScope(env, null);
    classNode.instanceFields.keys().forEach((fieldName) => {
      let field = classNode.instanceFields.get(fieldName);
      if (field.expression) {
  ...
  visitMethodInvocation(node) {
    let searchResult = methodResolver.resolve(node);
    if (!searchResult) {
