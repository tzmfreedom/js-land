...
const methodResolver = require('../resolver/build/method-resolver')
const variableResolver = require('../resolver/build/variable-resolver')
...
class ApexBuilder {
    ...
    if (node.modifiers) {
      node.modifiers.map((modifier) => {
        if (modifiers.includes(modifier.name)) {
          throw new Error(`Compile Error: duplicate modifier ${name} at line ${node.lineno}`)
        }
  ...
  visitClass (node) {
      ...
      methods.forEach((methodNode) => {
        ...
        if (methodNode.isOverride()) {
          ...
          if (superClassMethods.includes(methodName)) {
            const superClassMethod = getMatchedMethod(node.superClass.classNode, superClassMethods.get(methodName), methodNode.parameters)
    ...
    node.staticFields.keys().forEach((fieldName) => {
      ...
      if (expressionType && staticField.type.classNode != expressionType) {
        throw new Error(`Invalid type ${expressionType.name} at line ${node.lineno}`)
      }
    })
    ...
    node.implementClasses.forEach((interfaceClass) => {
      ...
      methods.keys().forEach((methodName) => {
        methods.get(methodName).forEach((method) => {
          ...
          const methodNode = getMatchedMethod(node, methodNodes, method.parameters)
          if (!methodNode) {
            this.throwError(node, method, `Class ${node.name} must implement the method`)
          }
        ...
        instanceMethods.keys().forEach((methodName) => {
          instanceMethods.get(methodName).forEach((method) => {
            if (method.isAbstract()) {
              ...
              const subClassMethodNode = getMatchedMethod(node, subClassMethodNodes, method.parameters)
  ...
  visitMethodDeclaration (node) {
    ...
    if (node.statements) {
      ...
      if (this.currentMethodNode.returnType.classNode !== null && !this.isReturn(returnNode)) {
        throw new Error(`Missing return statement required return type: ${this.currentMethodNode.returnType.name.join('.')} at line ${node.lineno}`)
      }
    }
  }
  ...
  createObject (classNode) {
    ...
    classNode.instanceFields.keys().forEach((fieldName) => {
      ...
      if (field.expression) {
        instanceFields.set(fieldName, new Ast.Variable(
          field.type,
  ...
  visitForControl (node) {
    ...
    if (condition.classNode != Runtime.Boolean) {
      throw new Error(`Should be boolean expression at line ${condition.lineno}`)
    }
  }
  ...
  visitIf (node) {
    ...
    if (condition.classNode != Runtime.Boolean) {
      throw new Error(`Should be boolean expression at line ${node.condition.lineno}`)
    }
    EnvManager.pushScope({})
  ...
  visitMethodInvocation (node) {
    ...
    if (!searchResult) {
      throw new Error(`Method missing ${node.nameOrExpression.value.join('.')} at line ${node.lineno}`)
    }
    const env = {}
    if (!(searchResult.receiverNode instanceof Ast.ApexClass)) {
      env.this = {
  ...
  visitName (node) {
    ...
    if (!resolveResult) {
      throw new Error(`Variable not declaration : ${node.value.join('.')} at line ${node.lineno}`)
    }

    if (resolveResult.receiverNode instanceof Ast.ApexClass) {
      const field = resolveResult.receiverNode.staticFields.get(resolveResult.key)
      if (!(field.isPublic())) {
        throw new Error(`Field is not visible : ${resolveResult.key} at line ${node.lineno}`)
      }
      return field.type
    } else {
  ...
  visitNew (node) {
    ...
    if (classNode.isAbstract()) {
      throw new Error(`Abstract classes cannot be constructed: ${node.classType.classNode.name}`)
    }
    const instanceFields = new CaseIgnoredStore()
    ...
    switch (node.op) {
      case '+':
        if (![Runtime.String, Runtime.Integer, Runtime.Double].includes(leftType.classNode)) {
          throw new Error(`Must be String or integer or double, but ${leftType.name} at line ${node.lineno}`)
        }
        if (![Runtime.String, Runtime.Integer, Runtime.Double].includes(rightType.classNode)) {
          throw new Error(`Must be String or integer or double, but ${rightType.name} at line ${node.lineno}`)
        }
        break
      case '-':
      case '*':
      ...
      case '>>':
        if (![Runtime.Integer, Runtime.Double].includes(leftType.classNode)) {
          throw new Error(`Must be integer or double, but ${leftType.name} at line ${node.lineno}`)
        }
        if (![Runtime.Integer, Runtime.Double].includes(rightType.classNode)) {
          throw new Error(`Must be integer or double, but ${rightType.name} at line ${node.lineno}`)
        }
        break
      case '&':
      case '|':
      case '^':
        if (leftType.classNode != Runtime.Integer) {
          throw new Error(`Must be integer, but ${leftType.name} at line ${node.lineno}`)
        }
        if (rightType.classNode != Runtime.Integer) {
          throw new Error(`Must be integer, but ${rightType.name} at line ${node.lineno}`)
        }
        break
      case '<':
      case '>':
      ...
      case '!==':
        if (!this.checkType(leftType, rightType)) {
          throw new Error(`Type not matched : left => ${leftType.name}, right => ${rightType.name} at line ${node.lineno}`)
        }
        break
      case '=':
      case '+=':
  ...
  visitReturn (node) {
    ...
    if (!this.checkType(this.currentMethodNode.returnType, node.type())) {
      throw new Error(`Illegal conversion from ${node.type().name.join('.')} to ${this.currentMethodNode.returnType.name.join('.')} at line ${node.lineno}`)
    }
    return node
  }
    ...
    node.declarators.forEach((declarator) => {
      ...
      if (!(declarator.expression instanceof Ast.SoqlNode)) {
        ...
        if (expressionType && !this.checkType(node.type, expressionType)) {
          throw new Error(`Type not matched : variable => ${node.type.classNode.name}, initializer => ${expressionType.classNode.name} at line ${node.lineno}`)
        }
      }
  ...
  visitVariableDeclarator (node) {
    ...
    if (EnvManager.localIncludes(name)) {
      throw new Error(`duplicate variable name ${name} at line ${node.lineno}`)
    }
  }
    ...
    for (let i = 0; i < node.statements.length; i++) {
      ...
      if (this.isReturn(returnNode)) {
        if (i < node.statements.length - 1) {
          throw new Error(`Unreachable statement ${node.statements[i].lineno + 1} at line ${node.lineno}`)
        }
      }
    }
  }
    ...
    switch (node.op) {
      case '!':
        if (expressionType.classNode !== Runtime.Boolean) {
          throw new Error(`Must be Boolean, but ${expressionType.name} at line ${node.lineno}`)
        }
        break
      case '+':
      case '-':
        if (![Runtime.Integer, Runtime.Double].includes(expressionType.classNode)) {
          throw new Error(`Must be integer or double, but ${expressionType.name} at line ${node.lineno}`)
        }
        if (![Runtime.Integer, Runtime.Double].includes(expressionType.classNode)) {
          throw new Error(`Must be integer or double, but ${expressionType.name} at line ${node.lineno}`)
        }
  ...
  visitTernalyExpression (node) {
    ...
    if (conditionClassNode !== Runtime.Boolean) {
      throw new Error(`Must be Boolean, but ${conditionClassNode.name} at line ${node.lineno}`)
    }
  }
  ...
  throwError (classNode, methodNode, message) {
    ...
    throw new Error(`${message}: ${typeName} ${classNode.name}.${methodNode.name}(${params})`)
  }
}
