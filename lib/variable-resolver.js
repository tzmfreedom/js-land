...
class VariableResolver {
  _resolve(node) {
    ...
    if (EnvManager.localIncludes(name)) {
      ...
      let receiverNode = this.reduce(variable, names.slice(1), conditionState, node)
    ...
    if (EnvManager.localIncludes('this')) {
      ...
      let receiverNode = this.reduce(thisReceiverNode, names, modifier.PRIVATE, node)
      if (receiverNode) return receiverNode
    }
    ...
    if (names.length == 1) {
      if (EnvManager.localIncludes('_class')) {
        ...
        if (classNode && classNode.staticFields.includes(name)) {
          ...
          let receiverNode = this.reduce(field, names.slice(1), modifier.PUBLIC, node)
          if (receiverNode) return receiverNode
        }
      }
    ...
    if (names.length > 1) {
      ...
      if (classNode && classNode.staticFields.includes(staticFieldName)) {
        ...
        const field = ClassStaticFields.get(classNode.name, staticFieldName);
        let receiverNode = this.reduce(field, names.slice(2), modifier.PUBLIC, node);
        if (receiverNode) return receiverNode;
      }
    }
    ...
    if (names.length > 2) {
      ...
      if (classNode) {
        ...
        if (classNode.staticFields.includes(staticFieldName)) {
          ...
          const field = ClassStaticFields.get(classNode.name, staticFieldName);
          let receiverNode = this.reduce(field, names.slice(3), modifier.PUBLIC, node);
          if (receiverNode) return receiverNode;
        }
      }
    }
