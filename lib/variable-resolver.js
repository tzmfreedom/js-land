...
const reduceTypeByInstanceField = (checkSetterGetter, isSetter) => {
  return (init, fieldNames, privateCheck, node) => {
    ...
    for (let i = 0; i < fieldNames.length - 1; i++) {
      ...
      const instanceField = searchInstanceField(receiverNode, fieldName)
    ...
    const instanceField = searchInstanceField(receiverNode, key)
...
const searchInstanceField = (receiverNode, fieldName) => {
  const classNode = (() => {
  ...
  if (classNode.instanceFields.includes(fieldName)) {
    const fieldNode = classNode.instanceFields.get(fieldName);
    if (fieldNode) return fieldNode
  }
  if (classNode.superClass) {
    const fieldNode = searchInstanceField(classNode.superClass, fieldName);
    if (fieldNode) return fieldNode
  }
}
...
const reduceValueByInstanceField = (init, fieldNames, node) => {
  ...
  for (let i = 0; i < fieldNames.length - 1; i++) {
    ...
    receiverNode = searchInstanceField(receiverNode, fieldName)
    if (!receiverNode) return null;
  }
  let key = fieldNames[fieldNames.length - 1];
  if (receiverNode instanceof Ast.NullNode) {
  ...
  searchFieldType(node, isSetter) {
    ...
    if (result.key) {
      if (result.receiverNode instanceof ApexClass) {
        ...
        const instanceField = searchInstanceField(receiverNode, result.key)
